...
def Prox(u,t):
    """
    Proximal operator for approximating gradient of non smooth function
    ST          = @(u, t) u .* max(0, 1 - t ./ abs(u));
    obj.myProxG = @(x, t) ST(x, t * mu2);
    Calculates R = lam* ||x||_1
    """
    return max(0, (1-t) / abs(u))
...
def Run_NIDS(alpha, k, nsz00, nsz10, nsx00, nsx10, nsmyfunMD_grad00):
    ...
    #ADD non smooth case and proximal operator
    nswk = A * nsw1
...
if __name__ == "__main__":
    main()
>>>>>>> fa6aea81495767e2d6afd762123d0f14b7ee966a
